	...
	return (
		<div id="sideBar" className={`h-[calc(100svh-4rem)] mt-16 py-5 px-3 border-r-1 border-gray-400 duration-500 ${isSidebarOpen == true ? "w-50" : "w-20"}`}>
			<ul className="flex flex-col items-center">
				...
				<li className={`capitalize flex items-center rounded-full mb-3 px-5 py-2 text-center ${currentPath == "user" ? "bg-gray-200" : ""} hover:bg-gray-200 ${isSidebarOpen == true ? "flex-row gap-2 w-full" : "justify-center w-12 h-12 flex-col text-xs align-middle"}`}
					onClick={() => navigate("/user")}>
					<FontAwesomeIcon icon={faUser} className={`w-8 ${isSidebarOpen ? "text-lg" : "text-xl"}`} />
